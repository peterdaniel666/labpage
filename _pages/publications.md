...
{% assign all_groups = pubs
  | group_by_exp: 'p', 'p.issued.first.year | default: 0'
  | sort: 'name'
  | reverse %}
  
  {% assign latest_year = nil %}
  {% for g in all_groups %}
  ...
  <div class="panel-group" id="pub-accordion" role="tablist" aria-multiselectable="true" markdown="0">
    ...
    <div class="panel-body">
      ...
      <p class="pub-item">
        {% if link_href %}
        <strong><a class="title-link" href="{{ link_href }}" target="_blank" rel="noopener">{{ publi.title }}</a></strong><br/>
        {% else %}
...
<style>
  #pub-accordion .pub-item { margin-bottom: .85em; }
